...
final class KeychainHelper {
    ...
    private let service = "com.cryptatext.passwordmanager"
    ...
    /// The context parameter is used for the prompt customization but the biometric
    /// authentication is handled automatically by the Keychain access control.
    func retrieveMasterPassword(context: LAContext) throws -> String {
        ...
        // The context is used for customizing the prompt, but we need to pass it
        // via a different mechanism. For now, we'll let the system handle it.
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: service,
            kSecAttrAccount as String: account,
