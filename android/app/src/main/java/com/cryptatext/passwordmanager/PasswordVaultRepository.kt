...
class PasswordVaultRepository(
    ...
    suspend fun addEntry(entry: PasswordEntry, masterPassword: String): Boolean {
        return kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            try {
                // Step 1: Get current entries (decryption on IO thread)
                val current = getCurrentEntries(masterPassword)
                
                android.util.Log.d("PasswordVault", "Current entries count: ${current.size}, adding entry: ${entry.service}")
                ...
                val encrypted = cryptoService.encrypt(jsonData, masterPassword, com.cryptatext.crypto.AesMode.AES_GCM)
     ...
     * Optimized: crypto operations happen outside dataStore.edit{} on IO dispatcher.
     */
    suspend fun updateEntry(entry: PasswordEntry, masterPassword: String) {
     ...
     * Optimized: crypto operations happen outside dataStore.edit{} on IO dispatcher.
     */
    suspend fun deleteEntry(id: String, masterPassword: String) {
        kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            try {
                // Step 1: Get current entries (decryption on IO thread)
                val current = getCurrentEntries(masterPassword)
    ...
    suspend fun changeMasterPassword(oldPassword: String, newPassword: String): Boolean {
        ...
        dataStore.edit { editPrefs ->
            if (entries.isEmpty()) {
                ...
                val encrypted = cryptoService.encrypt(jsonData, newPassword, com.cryptatext.crypto.AesMode.AES_GCM)
                editPrefs[PASSWORDS_KEY] = encrypted.encoded
