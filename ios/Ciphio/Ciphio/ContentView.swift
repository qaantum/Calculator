...
private let AlgorithmsInfo: [String] = [
                "Ciphio supports three AES block cipher modes to balance usability and security across different workflows.",
    ...
    "AES-CTR (Counter Mode) converts AES into a stream cipher using a 16-byte nonce. It is fast and avoids padding, but it does not provide built-in integrity—combine it with a signature or MAC when possible.",
                "Regardless of the mode, Ciphio derives a unique 256-bit key for each encryption by running PBKDF2-HMAC-SHA256 with 100,000 iterations and a random 16-byte salt."
]

private let TermsContent: [String] = [
                "Ciphio runs entirely on your device. No plaintext, keys, or history entries are transmitted to external servers.",
    ...
    "Encryption strength depends on the quality of the passphrases you choose. Use long, unique keys to maximise protection.",
                "By using Ciphio you acknowledge that the developers are not liable for data loss, misuse, or damages arising from its usage.",
