...
struct ContentView: View {
    ...
    private var palette: CryptatextPalette {
        CryptatextPalette.palette(for: resolvedScheme)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(isPresented: $showSettings) {
                ...
                .navigationDestination(isPresented: $showAlgorithms) {
                    InfoView(title: "Encryption Algorithms", paragraphs: AlgorithmsInfo)
                }
                .navigationDestination(isPresented: $showTerms) {
                    InfoView(title: "Terms of Service", paragraphs: TermsContent)
                }
            }
        }
        .environment(\.cryptatextPalette, palette)
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    .sheet(isPresented: $showPasswordManager) {
                        PasswordManagerView(cryptoService: CryptoService())
                    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Slider(value: $viewModel.passwordLength, in: 6...64, step: 1)
                    .onChange(of: viewModel.passwordLength) { oldValue, newValue in
                        ...
                        HStack {
                            VStack(alignment: .leading) {
                                Text("Upgrade to Premium")
                                    .font(.headline)
                                    .foregroundColor(palette.primary)
                                Text("Unlock unlimited history & more")
                                    .font(.caption)
                                    .foregroundColor(palette.mutedForeground)
                            }
                            Spacer()
...
private let AlgorithmsInfo: [String] = [
    "Cryptatext supports three AES block cipher modes to balance usability and security across different workflows.",
    ...
    "Regardless of the mode, Cryptatext derives a unique 256-bit key for each encryption by running PBKDF2-HMAC-SHA256 with 100,000 iterations and a random 16-byte salt."
]

private let TermsContent: [String] = [
    "Cryptatext runs entirely on your device. No plaintext, keys, or history entries are transmitted to external servers.",
    "You are responsible for safeguarding the secrets and ciphertexts produced by the application.",
    "Encryption strength depends on the quality of the passphrases you choose. Use long, unique keys to maximise protection.",
    "By using Cryptatext you acknowledge that the developers are not liable for data loss, misuse, or damages arising from its usage.",
    "Continued use after updates constitutes acceptance of any revised terms and policies."
]
